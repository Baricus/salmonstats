...
statMapsSum = fmap (foldl' (+) 0) . SM.toStatsList
...
statMapsAvg m = let len = length m
          in fmap ((/ fromIntegral len) . fromIntegral . foldl' (+) 0) . SM.toStatsList $ m
...
calcMedian :: (Integral a, Ord a, Num b, Fractional b) => [a] -> Maybe b
calcMedian [] = Nothing
